...
@Provider
public class USerSecurity_Filter implements ContainerRequestFilter {
			...
			if (AUTHENTICATION_HEADER != null && AUTHENTICATION_HEADER.size() > 0) {
				...
				if (m.isAnnotationPresent(RolesAllowed.class)) {
					...
					if (user_role.contains("admin")) {
						wt = c.target("http://localhost:8090/authentification/Service").path("usersAuth");
					} 
					else if (user_role.contains("buyer")) {
						wt = c.target("http://localhost:8090/authentification/Service").path("usersAuth");

					} else {
						wt = c.target("http://localhost:8090/authentification/Service").path("usersAuth/deny");
					}
